...
export default class Segments extends Iterable {
	...
	async splice(segment, offset, duration) {
		...
		try {
			...
			return Segment.fromObject({
				path: `${this.path}/${id}.mkv`,
				date: segment.date + offset
			});
		}
		catch(err) {
			await fs.rm(`${this.path}/${id}.mkv`, { force: true });
			throw err;
		}
	}
	async concat(...segments) {
		...
		try {
			await fs.writeFile(`${this.path}/${id}.txt`, `${segments.map(segment => `file '${segment.path}'`).join('\n')}\n`);
			...
			return Segment.fromObject({
				path: `${this.path}/${id}.flv`,
				date: segments[0].date
			});
		}
		catch(err) {
			await fs.rm(`${this.path}/${id}.flv`, { force: true });
			throw err;
		}
		finally {
			await fs.rm(`${this.path}/${id}.txt`, { force: true });
		}
	}
	async clip(start, stop) {
		...
		try {
			...
			concatSegment = await this.concat(...segments);
