...
export default class Segment {
	...
	async getMediaInfo() {
		...
		mediainfo.stderr.on('data', buff => console.error(`${this.name} mediainfo stderr: ${buff.toString()}`));
		const outBuffs = [];
	...
	async repair() {
		...
		console.log(`Repairing segment: ${this.path}`);
		try {
			...
			ffmpeg.stderr.on('data', buff => console.error(`${this.name} ffmpeg repair stderr: ${buff.toString()}`));
			await new Promise((resolve, reject) => {
