...
import { DEFAULT_FFMPEG_PATH, DEFAULT_MEDIAINFO_PATH } from './constants.js';
...
export default class Segment extends EventEmitter {
	...
	#truncated;
	#segmentId;
	constructor(db, name, path, tempPath, ffmpegPath = DEFAULT_FFMPEG_PATH, mediainfoPath = DEFAULT_MEDIAINFO_PATH, date = 0, duration = 0, bytes = 0, truncated = true, segmentId = null) {
	...
	async getMediaInfo() {
		...
		const mediainfo = spawn(this.mediainfoPath, args);
		this.emit('processStart', new ProcessToken(mediainfo.pid, 'mediainfo'));
		...
		await new Promise((resolve, reject) => {
			mediainfo.on('close', code => {
				this.emit('processStop', new ProcessToken(mediainfo.pid, 'mediainfo'));
				if (code !== 0)
	...
	async repair() {
		...
		try {
			...
			const args = [
				...
				'-i', this.path,
				'-c', 'copy',
				`${this.tempPath}/${this.nameSanitized}/${fileName}-repaired.mkv`
			];
			...
			await fs.copyFile(`${this.tempPath}/${this.nameSanitized}/${fileName}-repaired.mkv`, this.path);
			await new Promise(resolve => setTimeout(resolve, 100));
			return true;
		}
		catch (err) {
			...
			// attempt to clean up
			try { await fs.unlink(`${this.tempPath}/${this.nameSanitized}/${fileName}-repaired.mkv`); }
			catch (err) { /* nope! */ }
		}
	}
	toJSON() {
