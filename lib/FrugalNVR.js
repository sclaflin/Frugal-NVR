...
export default class FrugalNVR {
	...
	#mqtt;
	#pruneTimeout;
	#processTokens = [new ProcessToken(process.pid, 'node')];
	...
	constructor(cameras, db, webUI, nodeMediaServer, mqtt) {
		...
		this.#mqtt = mqtt;
		this.webUI.processTokens = this.#processTokens;
		this.cameras.on('processStart', processToken => this.#processTokens.push(processToken));
		this.cameras.on('processStop', processToken => {
			const index = this.#processTokens.findIndex(v => v.pid === processToken.pid);
			if (index >= 0)
				this.#processTokens.splice(index, 1);
		});
			...
			const camera = await Camera.fromObject(deepmerge(
				{
					...
					mediainfoPath: config?.mediainfoPath,
