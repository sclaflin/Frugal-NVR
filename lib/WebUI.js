...
export default class WebUI {
	...
	set pids(v) {
		if (!Array.isArray(v))
			throw new TypeError('pids must be an array.');
		if (v.filter(pid => !Number.isInteger(pid)) > 0)
			throw new TypeError('all items of pids must be an integer.');
		this.#pids = v;
	}
	get cameras() {
		return this.#cameras;
			...
			if (obj.type === 'WebSocketRequest') {
				...
				switch (request.command) {
					...
					case 'frugal.getStats': {
						const stats = await getStats(...this.pids);
