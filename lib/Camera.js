...
export default class Camera extends EventEmitter {
	...
	constructor(name, videoPath, stillsPath, watcher, cam, retainHours = constants.CAM_DEFAULT_RETAIN_DAYS, generateThumbs = constants.CAM_DEFAULT_GENERATE_THUMBS) {
		if (typeof name !== 'string')
			throw new TypeError('name must be a string.');
		if (typeof videoPath !== 'string')
			throw new TypeError('videoPath must be a string.');
		if (typeof stillsPath !== 'string')
			throw new TypeError('stillsPath must be a string.');
	...
	toJSON() {
		return {
			...
			stillsPath: this.#stillsPath,
			retainHours: this.#retainHours
		};
	}
	async processWatcherEvent(path) {
		const regex = new RegExp(`^${this.stillsPath}/${this.nameSanitized}/([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2})\\.jpg$`);
		if (!regex.test(path))
			return;
		try {
	...
	async start() {
		try {
			...
			await fs.mkdir(`${this.stillsPath}/${this.nameSanitized}`, { recursive: true });
			...
			if (this.generateThumbs) {
				...
				// start paying attention to the stills path
				this.watcher.add(this.stillsPath);
			}
			...
			if (this.generateThumbs) {
				const grabberArgs = [
					...
					'-strftime', '1',
					// output dat file
					`${this.stillsPath}/${this.nameSanitized}/%Y-%m-%dT%H:%M:%S.jpg`,
				];
	...
	stop() {
		...
		this.watcher.unwatch(this.stillsPath);
