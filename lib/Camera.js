...
export default class Camera extends EventEmitter {
	...
	constructor(
		...
		this.segments.on('processStart', pid => this.emit('processStart', pid));
		this.segments.on('processStop', pid => this.emit('processStop', pid));
	...
	#processWatcherEvent = async (event, filePath) => {
		// handle thumbnails
		const imageRegex = new RegExp(`^/dev/shm/${this.nameSanitized}/([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2})\\.jpg$`);
		if (imageRegex.test(filePath)) {
			if (event !== 'add') return;
			setTimeout(async () => {
				try {
					this.#thumb = await sharp(filePath)
		...
		if (segmentRegex.test(filePath)) {
			if (event === 'add') {
				...
				this.#segmentChangeTimeout = setTimeout(async () => {
					try {
						const segment = this.segments.items.find(segment => segment.path === filePath);
						...
						console.error(`Failed to update ${filePath}: ${err.stack}`);
					}
					finally {
	...
	#processCamEvent = async (msg) => {
		try {
			...
			if (event.topic === 'VideoSource/MotionAlarm' && event.property === 'Changed') {
				const isMotion = event.data.find(v => v.name === 'State').value;
				if (isMotion) {
				...
				// send
				this.webUI?.sendEvent(WebSocketEvent.fromObject({
					name: `motionEvent.${isMotion ? 'start' : 'stop'}`,
					data: {
						name: this.name,
						date: Math.round(event.time.getTime() / 1000) + (isMotion ? -constants.CAM_DEFAULT_TIME_PADDING : constants.CAM_DEFAULT_TIME_PADDING)
					}
				}));
			}
		}
	...
	async start() {
		try {
			...
			if (this.#running)
				this.stop();
			this.#running = true;
			// make sure the shared memory path exists
			...
			if (this.generateThumbs) {
				...
				this.watcher.add(`/dev/shm/${this.nameSanitized}`);
			}
			this.#ffmpegSplitter = spawn('ffmpeg', splitterArgs);
			this.emit('processStart', this.#ffmpegSplitter.pid);
			this.#ffmpegSplitter.stderr.on('data', buff => console.error(`${this.name} FFMPEG splitter stderr: ${buff.toString()}`));
			this.#ffmpegSplitter.on('close', code => {
				this.emit('processStop', this.#ffmpegSplitter.pid);
			...
			if (this.generateThumbs) {
				...
				this.emit('processStart', this.#ffmpegFrameGrabber.pid);
				this.#ffmpegFrameGrabber.stderr.on('data', buff => console.error(`${this.name} FFMPEG grabber stderr: ${buff.toString()}`));
				this.#ffmpegFrameGrabber.on('close', code => {
					this.emit('processStop', this.#ffmpegFrameGrabber.pid);
	...
	static async fromObject(config) {
		...
		console.log(`Connecting to ${config.name} via ONVIF...`);
		...
		console.log(`Querying ${config.name} for capabilities...`);
		...
		console.log(`Initializing ${config.name} file watcher...`);
